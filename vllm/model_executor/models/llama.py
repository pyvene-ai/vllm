...
class LlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3):
    ...
    def __init__(self,
        ...
        if layer_type is LlamaDecoderLayer:
            ...
            if adapter_spec is not None:
                layer_type = make_adapter_llama_layer(adapter_spec)
            elif enable_adapters:
                layer_type = make_adapter_llama_layer(None)
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        loaded = loader.load_weights(
            ...
            for name, loaded_weight in weights)
        # adapter params are initialized from the blueprint spec,
    ...
    def refresh_adapter_caches(self) -> None:
        ...
        for layer in self.model.layers:
            if not hasattr(layer, "served_adapters"):
                continue
            for adapter in layer.served_adapters.values():
